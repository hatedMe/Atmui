...
let listenTouchEvent = function(el, type, handler , isDetach) {
    let params = {};
    let touches = {
        direction: 0,
        vertical: 0,
    ...
    let onTouchEnd=function(e){
        ...
        //执行函数
        for(var n in el.touchEvents){
            if(eventName===n)el.touchEvents[n](e);
        }
    };

    let attach=function(){
        if(Type.isEmptyObject(el.touchEvents)){
            el.touchEvents={};
            el["addEventListener"]("touchstart",onTouchStart,false);
            el["addEventListener"]("touchend",onTouchEnd,false);
        }
        el.touchEvents[type]=handler;
    };
    //移除事件
    let detach=function(){
        if(el.touchEvents)delete el.touchEvents[type];
        if(Type.isEmptyObject(el.touchEvents)){
            console.log(el.touchEvents);
