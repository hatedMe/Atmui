...
class Picker {
    ...
    addSolt(values, defaultKey) { //添加一列
    ...
    onTouchStart(e) {
        ...
        //if (this.params.onScrollStart) this.params.onScrollStart(s);  // Callback
    }
    onTouchMove(e) {
        e.preventDefault();
        ...
        this.activeSlot.style.webkitTransform = 'translate(0px,' + this.touches.currentPosY + 'px)';
        //if (s.params.onScroll) s.params.onScroll(s); // Callback
    }
    onTouchEnd(e) {
        //if (this.activeSlot.isLock) return;// 锁定的槽将不工作
        ...
        this.activeSlot.style.webkitTransform = 'translate(0px,' + inertance.value + 'px)'

        //if (s.params.onScrollEnd) s.params.onScrollEnd(s);        // Callback onScrollEnd
    }
    ...
    onTransitionEnd(e) {
        ...
        if (target.classList.contains(this.opt.pickerClass)) {
            // if (this.opt.onTransitionEnd) this.opt.onTransitionEnd(s)
            if (this.isHid) {
    ...
    updateActiveOptions(slot, posY) { // 更新选中项目
        ...
        console.log(this.activeOptions, '选中项');
    }
